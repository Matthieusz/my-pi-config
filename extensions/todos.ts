...
class TodoSelectorComponent extends Container implements Focusable {
    ...
    handleInput(keyData: string): void {
        ...
        if (matchesKey(keyData, Key.ctrlShift("w"))) {
            const selected = this.filteredTodos[this.selectedIndex];
            if (selected && this.onQuickAction)
                this.onQuickAction(selected, "work");
            return;
        }
        ...
        this.addChild(
            new Text(
                theme.fg(
                    "accent",
                    theme.bold(
                        `Actions for ${formatTodoId(todo.id)} "${title}"`,
                    ),
                ),
            ),
        );
    ...
    private buildTitleLine(width: number): string {
        ...
        if (titleWidth >= width) {
            return truncateToWidth(
                this.theme.fg("accent", titleText.trim()),
                width,
            );
        }
            ...
            switch (action) {
                ...
                case "list-all": {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: serializeTodoListForAgent(todos),
                            },
                        ],
                        details: {
                            action: "list-all",
                ...
                case "get": {
                    if (!params.id) {
                        return {
                            content: [
                                { type: "text", text: "Error: id required" },
                            ],
                    ...
                    return {
                        content: [
                            { type: "text", text: serializeTodoForAgent(todo) },
                        ],
                ...
                case "create": {
                    if (!params.title) {
                        return {
                            content: [
                                { type: "text", text: "Error: title required" },
                            ],
                            details: {
                                action: "create",
                ...
                case "update": {
                    ...
                    const result = await withTodoLock(
                        ...
                        ctx,
                        async () => {
                            const existing = await ensureTodoExists(
                                filePath,
                                normalizedId,
                            );
                            if (!existing)
                                return {
                                    error: `Todo ${displayId} not found`,
                                } as const;
                ...
                case "delete": {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: serializeTodoForAgent(
                                    result as TodoRecord,
                                ),
                            },
                        ],
                        details: {
            ...
            await ctx.ui.custom<void>((tui, theme, keybindings, done) => {
                ...
                const copyTodoPathToClipboard = (todoId: string) => {
                    ...
                    try {
                        ...
                        ctx.ui.notify(
                            `Copied ${absolutePath} to clipboard`,
                            "info",
                        );
                    } catch (error) {
                        const message =
                ...
                const resolveTodoRecord = async (
                    ...
                    if (!record) {
                        ctx.ui.notify(
                            `Todo ${formatTodoId(todo.id)} not found`,
                            "error",
                        );
                        return null;
                    }
                ...
                const showActionMenu = async (
                    ...
                    const record =
                        "body" in todo ? todo : await resolveTodoRecord(todo);
                    if (!record) return;
