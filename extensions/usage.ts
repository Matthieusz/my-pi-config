...
function addSessionToRange(range: RangeAgg, session: ParsedSession): void {
    ...
    if (cwd) {
        ...
        day.tokensByCwd.set(
            cwd,
            (day.tokensByCwd.get(cwd) ?? 0) + session.tokens,
        );
        range.cwdTokens.set(
            cwd,
            (range.cwdTokens.get(cwd) ?? 0) + session.tokens,
        );
        day.costByCwd.set(
            cwd,
            (day.costByCwd.get(cwd) ?? 0) + session.totalCost,
        );
        range.cwdCost.set(
            cwd,
            (range.cwdCost.get(cwd) ?? 0) + session.totalCost,
        );
    }
...
function renderDowDistributionLines(
    ...
    const valueWidth = kind === "tokens" ? 10 : 8;
    const showValue =
        width >= dayWidth + 1 + 10 + 1 + pctWidth + 1 + valueWidth;
    const fixedWidth =
    ...
    for (const dow of DOW_NAMES) {
        ...
        const emptyBar =
            empty > 0 ? ansiFg(EMPTY_CELL_BG, "█".repeat(empty)) : "";
...
class BreakdownComponent implements Component {
    ...
    handleInput(data: string): void {
        ...
        if (
            ...
            this.view =
                views[(idx + views.length + dir) % views.length] ?? "model";
            this.invalidate();
            this.tui.requestRender();
            return;
        }
    ...
    render(width: number): string[] {
        ...
        if (this.view === "model") {
            ...
            for (const cwd of this.data.cwdPalette.orderedCwds) {
                ...
                if (c)
                    legendItems.push(
                        `${ansiFg(c, "█")} ${abbreviatePath(cwd, 30)}`,
                    );
            }
            legendItems.push(`${ansiFg(activeOtherColor, "█")} other`);
        ...
        if (this.view === "dow") {
            ...
            const maxScale =
                selectedDays === 7 ? 4 : selectedDays === 30 ? 3 : 2;
        ...
        if (this.view === "dow") {
            ...
            if (showSideLegend) {
                ...
                for (const gl of graphLines)
                    lines.push(truncateToWidth(gl, width));
                lines.push("");
                ...
                for (const it of legendItems)
                    lines.push(truncateToWidth(it, width));
            }
        }
