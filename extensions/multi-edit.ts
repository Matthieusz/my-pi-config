    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (part.added || part.removed) {
            ...
            for (const line of raw) {
                if (part.added) {
                    const lineNum = String(newLineNum).padStart(
                        lineNumWidth,
                        " ",
                    );
                    ...
                    const lineNum = String(oldLineNum).padStart(
                        lineNumWidth,
                        " ",
                    );
                    output.push(`-${lineNum} ${line}`);
...
async function applyPatchOperations(
    ...
    for (const op of ops) {
        ...
        if (op.kind === "delete") {
            ...
            if (!exists) {
                throw new Error(
                    `Failed to delete ${op.path}: file does not exist`,
                );
            }
            let oldText = "";
            if (collectDiff) {
    ...
    for (const absPath of editOrder) {
        ...
        if (group.length > 1) {
            ...
            for (const entry of group) {
                ...
                positions.set(
                    entry,
                    pos === -1 ? Number.MAX_SAFE_INTEGER : pos,
                );
            }
